...
<div class="d-flex justify-content-end">
  <?php
    if (!empty($_SESSION['cart'])) {
      echo <<< _END
        <form action="/bookstore/cart/buy" method="POST">
        <button type="submit" name="getorder" class="btn btn-success mx-4 mb-2">Xác nhận mua sách</button>
        </form>
      _END;
    } else {
      echo <<< _END
      <a href="/bookstore/book"><button type="button" class="btn btn-success mx-4 mb-2">Xác nhận mua sách</button></a>
      _END;
    }
  ?>
</div>
...
<?php		
    foreach ($_SESSION['cart'] as $item){
		...
		?>
          ...
          <form action="/bookstore/cart/remove" method="POST"> 
